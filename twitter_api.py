...
class Bot(object):
    ...
    def get_follower_ids(self, screen_name):
        ...
        follower_ids = []
        for fid in Cursor(self.api.followers_ids, screen_name=screen_name, count=50).items():
    ...
    def get_friend_ids(self, screen_name):
        ...
        friend_ids = []
        for fid in Cursor(self.api.friends_ids, screen_name=screen_name, count=50).items():
    ...
    def get_user_tweets(self, _screen_name):
        tweets = [];
        retryCount = 10;
        interval = 60;
        while(retryCount >= 0): 
            try:  
                for tweet in Cursor(self.api.user_timeline, screen_name=_screen_name, exclude_replies=True, count = 50).items():                      
                    print(tweet)
                    tweets.append(tweet) 
                break; 
            except:  
                print("Got exception, will rest for " + str(interval) + "s")
                time.sleep(interval)
                interval *= 2
                retryCount -=1;
